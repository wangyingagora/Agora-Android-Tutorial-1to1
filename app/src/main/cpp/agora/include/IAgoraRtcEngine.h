...
public:
  ...
  * @param [in] info
    ...
    * @return return 0 if success or an error code
    */
    virtual int joinChannel(const char* token, const char* channelId, const char* info, uid_t userId) = 0;
    ...
    */
    ...
    /**
    * enable audio function, which is enabled by default.
    * @return return 0 if success or an error code
    */
...
public:
  ...
  /**
    ...
    * @return return 0 if success or an error code
    */
    int muteRemoteAudioStream(uid_t userId, bool mute) {
        return setObject("rtc.audio.mute_peer", "{\"uid\":\"%u\",\"mute\":%s}", userId, mute ? "true" : "false");
    }
    ...
    * @param [in] userId
    *        the userId of the remote user you want to mute/unmute
    * @param [in] mute
    *        true:  mute
    *       false: unmute
    * @return return 0 if success or an error code
    */
    int muteRemoteVideoStream(uid_t userId, bool mute) {
        return setObject("rtc.video.mute_peer", "{\"uid\":\"%u\",\"mute\":%s}", userId, mute ? "true" : "false");
    }

    int setRemoteVideoStreamType(uid_t userId, REMOTE_VIDEO_STREAM_TYPE streamType) {
        return setObject("rtc.video.set_remote_video_stream", "{\"uid\":\"%u\",\"stream\":%d}", userId, streamType);
    }
    ...
    */
    int setRemoteRenderMode(uid_t userId, RENDER_MODE_TYPE renderMode) {
        return setObject("rtc.api.set_remote_render_mode", "{\"uid\":\"%u\",\"mode\":%d}", userId, renderMode);
    }
    
    /**
    * set userId compatible mode
    * @param [in] enabled
...
protected:
    agora::base::AParameter& parameter() {
        return m_parameter;
    }
...
private:
    agora::base::AParameter m_parameter;
};
